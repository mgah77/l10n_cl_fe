...
class UploadXMLWizard(models.TransientModel):
    ...
    def get_product_values(self, line, company_id, price_included=False,
        ...
        _logger.warning(f"get_product_values. DEBUG")  # Línea añadida para debug
        amount = 0
        sii_code = 0
    ...
    def _procesar_po_to_done(self, vals, company_id):
        ...
        origen = vals.get('origen', '0')  # Valor por defecto '0' si no existe
        origen_limpio = origen.strip().upper().replace(seq.prefix, '').replace(' ', '')
        
        # Si el valor no es numérico, usar 0 o saltar la búsqueda
        if not origen_limpio.replace(".", "").isdigit():  # Permite números con puntos (ej: "123.45")
            origen_limpio = "0"  # O manejarlo de otra forma (ej: return early)
        
        try:
            folio_numero = int(float(origen_limpio))  # Convierte a float primero por si acaso
        except (ValueError, AttributeError):
            folio_numero = 0  # Valor por defecto si falla la conversión
        
        self.purchase_to_done = self.env['purchase.order'].search([
            ('name', '=', seq.get_next_char(folio_numero))
        ])
    ...
    def process_dr(self, dr, journal_id=False):
        ...
        if journal_id:
            ...
            if not data['account_id']:
                _logger.warning(f"process_dr. DEBUG")  # Línea añadida para debug
    ...
    def _get_journal(self, sii_code, company_id, ignore_journal=False):
        ...
        _logger.warning(f"get journal . siicode:{sii_code} , dc_id :{dc_id}")  # Línea añadida para debug        
        type = "purchase"
        query = [("company_id", "=", company_id.id),]
        if self.type == "ventas":
            type = "sale"
            _logger.warning(f"type = sale , busca donde journal_document_class_ids.sii_document_class_id = {dc_id}")  # Línea añadida para debug
            query.append(("journal_document_class_ids.sii_document_class_id", "=", dc_id.id))
        else:
            query.append(("document_class_ids", "=", dc_id.id))
            _logger.warning(f"type no era ventas, busca document_class_ids={dc_id}")  # Línea añadida para debug
        query.append(("type", "=", type))
        _logger.warning(f"type = {type}")  # Línea añadida para debug
        journal_id = self.env["account.journal"].search(
                query, limit=1,
            )
        if not journal_id and not ignore_journal:
            _logger.warning(f"get_journal. DEBUG")  # Línea añadida para debug
            raise UserError(
       ...
       #logica anterior
        ...
        if not self.crear_po:
            ...
            if tipo_dte == "34" or exenta_por_totales:
                ...
                iva_calculado = float_round(
                    subtotal_afecto_ajustado * 0.19, 
                    precision_digits=0, 
                    rounding_method='HALF-UP'  # <--- Esta es la clave
                )
    ...
    def do_create_inv(self):
        ...
        for dte in dtes:
            try:
                ...
                company_id = self.env["res.company"].search(
                    [("vat", "=", self.format_rut(rut)),], limit=1,
                )
                if not company_id:
