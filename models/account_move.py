...
class AccountMove(models.Model):
    ...
    claim_ids = fields.One2many("sii.dte.claim", "move_id", string="Historial de Reclamos")
    ...
    def _set_next_sequence(self):
        ...
        _logger.info("============================================")
        _logger.warning("DEBUG _set_next_sequence Iniciado para ID: %s", self.id)
        _logger.warning("Diario Actual: %s (ID: %s)", self.journal_id.name, self.journal_id.id)
        _logger.warning("¿Usa Documentos?: %s", self.use_documents)
        _logger.warning("Journal Document Class ID: %s", self.journal_document_class_id)
        
        if self.journal_document_class_id:
            _logger.warning("  -> Secuencia ID configurada: %s", self.journal_document_class_id.sequence_id)
            if self.journal_document_class_id.sequence_id:
                _logger.warning("  -> Nombre de la Secuencia: %s", self.journal_document_class_id.sequence_id.name)
            else:
            ...
            if self.journal_document_class_id.sii_document_class_id:
                _logger.warning("  -> Tipo de Doc SII: %s", self.journal_document_class_id.sii_document_class_id.name)
        else:
        ...
        if self.use_documents and self.journal_document_class_id.sequence_id:
            if not (self.journal_id.restore_mode or self._context.get("restore_mode", False)):
                _logger.warning("Ejecutando sequence_id.next_by_id()...")
                self.sii_document_number = self.journal_document_class_id.sequence_id.next_by_id()
                _logger.warning("Número obtenido sii_document_number: %s", self.sii_document_number)
            else:
            ...
            _logger.warning("Nombre final del documento (campo name): %s", self[self._sequence_field])
        else:
            _logger.warning("No se cumplen condiciones para secuencia SII, usando método estándar (super)...")
    ...
    def _post(self, soft=True):
        ...
        to_send = {}
        for inv in to_post:
            if not inv.is_invoice() or not inv.journal_document_class_id.sequence_id or not inv.use_documents or inv.state != 'posted':
                continue
    ...
    def _get_sequence_number_next(self):
        for invoice in self:
            invoice.sequence_number_next = -1
            if invoice.journal_document_class_id and invoice.journal_document_class_id.sequence_id:
    ...
    def copy(self, default=None):
        ...
        if default is not None and default["referencias"]:
    ...
    def action_convert_to_dte(self):
        action = self.env["ir.actions.actions"]._for_xml_id("l10n_cl_fe.action_convert_to_dte")
        return action
    ...
    def _receptor(self):
        ...
        if not self.es_boleta() and not self.es_nc_boleta():
            GiroRecep = (
                self.acteco_id.name
                or commercial_partner_id.activity_description.name
                or (self.acteco_ids and self.acteco_ids[0].name)
            )
            if not GiroRecep:
    ...
    def _totales(self, resumen):
        ...
        if not resumen['product']:
            return totales
        totales["MontoNF"] = resumen['MontoNF']
        ...
        if self._es_exento():
            ...
            for t in self.line_ids:
                if t.display_type == 'rounding' or t.ind_exe:
                    continue
                if another_currency_id and currency_base != another_currency_id:
                    balance = sign * t.amount_currency
                else:
        ...
        totales['MntTotal'] = totales['MntNeto'] + totales['MntExe'] + \
            totales['MntIVA'] + totales['OtrosImp'] - totales['CredEC'] \
            - totales['MntRet']  - val_com_neto \
            - val_com_exe - val_com_iva
        if totales['MontoNF']:
            if self.es_boleta():
                totales['TotalPeriodo'] = totales['MntTotal'] + totales['MontoNF']
            else:
                totales['MontoPeriodo'] = totales['MntTotal'] + totales['MontoNF']
    ...
    def _get_lineas(self):
        return self.with_context(lang="es_CL").invoice_line_ids
    ...
    def _detalle(self):
        ...
        desglose = self._context.get('force_desglose', False)
        for line in self._get_lineas():
            ...
            details = line.get_tax_detail(desglose)
            if len(detalle) > 0 and not desglose and details['desglose']:
                """ Fuerzo desglose para todo el contexto """
                return self.with_context(force_desglose=True)._detalle()
            desglose = desglose or details['desglose']
            ...
            if lines.get("PrcItem", 1) == 0:
                del lines["PrcItem"]
            detalle.append(lines)
        if self.invoice_cash_rounding_id:
            cash_rounding = self.line_ids.filtered(lambda l: l.display_type=='ajuste')
            if cash_rounding:
